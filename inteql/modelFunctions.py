...
def random_forest_regressor(data, X_label, y_label, random_state, test_size=0.3, max_depth=83, n_estimators=136,
...
def random_forest_regressor_fixed_split(traindata, testdata, X_label, y_label, max_depth=83, n_estimators=136,
                                        min_samples_split=2, min_samples_leaf=1, max_features='sqrt'):
    """ todo rewrite doc
    This function generate a Random Forest Regressor.
    This variant uses predefined test and train data instead of splitting randomly.
    Then, trains the model.
    Finally, predicts the y values for the test set and compute its error.
    It also computes the regression line for the test and predictions.
    It receives:
    ...
    # Split data
    X_train = traindata[X_label]
    y_train = traindata[y_label]
    X_test = testdata[X_label]
    y_test = testdata[y_label]
