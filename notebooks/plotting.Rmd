...
library(tidyr)
library(ggpubr)
setwd('/mnt/c/Users/laura/Documents/EBI_internship/TFG/github/notebooks/')
```
...
data <- read.csv('../data/data_figure2.csv')
head(data)
...
ggsave('../figures/mean_vs_real.png', p)
...
```{r}
RMSE_data <- read.csv('../../data_RMSE_figure3.tab', sep='\t')
head(RMSE_data)


rval_data <- read.csv('../../data_rval_figure3.tab', sep='\t')
head(rval_data)
...
p
ggsave('../figures/imputation_box.png', p)
...
# Figure 4
```{r}
colors_df <- read.csv('../data/GTEx_v7_Annotations_ColorTissues.csv', stringsAsFactors = F)
head(colors_df)
...
```{r}
tissues_id_eqtl <- colnames(read.csv('../data/GTEx_Analysis_v7_eQTL_allTissues_slope_top_complete.csv.gz'))
...
```{r}
tissues_id_rna <- scan('../data/GTEx_v7_Annotations_TissuesId.txt', what = 's', sep='\n')
```
...
```{r}
pc_centroids_rna = read.csv('../data/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_pc_centroids.csv')
