...
for i in $chromosomes; do
  if [ -e "$perchroutput09"output09_$i.csv.gz ]; then
    awk -F $'\t' -v CHR="$i" '{if(NR==2&&CHR==1) print $1 FS $2 FS $3 FS $4 FS $5 FS "Chromosome"; else if(NR>=3&&NR<=9) print $1 FS $2 FS $3 FS $4 FS $5 FS CHR}' $perchroutput10"output10_$i.csv"
  fi
done >$output10
awk -F $'\t' '{if(NR>=3&&NR<=9) print $1 FS $2 FS $3 FS $4 FS $5 FS "All"}' $perchroutput10'output10_ALL.csv' >> $output10
